const PYUSD = artifacts.require('PYUSDImplementation.sol');
const Proxy = artifacts.require('AdminUpgradeabilityProxy.sol');

const assertRevert = require('./helpers/assertRevert');
const {ZERO_ADDRESS} = require('@openzeppelin/test-helpers').constants;

// Test that PYUSD operates correctly as an Ownable token.
contract('Ownable PYUSD', function ([_, admin, anotherAccount, anotherAccount2, owner]) {
    beforeEach(async function () {
        const pyusd = await PYUSD.new({from: owner});
    ...
    describe('as an initializable token', function () {
        ...
        it('constructor initializes the implementation contract and pauses it to avoid misleading state there', async function () {
            const isPaused = await this.pyusd.paused();
            assert.strictEqual(isPaused, true);
            const currentOwner = this.pyusd.owner();
            assert.notStrictEqual(currentOwner, ZERO_ADDRESS);
            await assertRevert(this.pyusd.initialize());
        });
    });
});
